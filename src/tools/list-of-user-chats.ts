...
export async function listOfUserChatsHandler(
  input: any,
  accessKey: string,
  secretKey: string
) {
  const result = await makeOpenApiRequest<any>(
    `/open/v5/user-chats?state=${input.state}&limit=100`,
    accessKey,
    secretKey
  );
  ...
  // 채팅 주요 정보
  const mappedUserChats = result.userChats.map((uc: any) => {
    return {
      id: uc.id, // 채팅 아이디
      state: uc.state, // 상태
  ...
  // 관리자 정보
  const mappedMangers = result.managers.map((m: any) => {
    return {
      id: m.id,
      name: m.name,
    };
