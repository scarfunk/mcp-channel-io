...
export function getConfig() {
    ...
    const ACCESS_KEY = cliArgs["access-key"] || process.env.CHANNEL_TALK_X_ACCESS_KEY;
    const SECRET_KEY = cliArgs["secret-key"] || process.env.CHANNEL_TALK_X_ACCESS_SECRET;
    if (!ACCESS_KEY || !SECRET_KEY) {
        console.error("❌ 오류: ACCESS_KEY 또는 SECRET_KEY가 설정되지 않았습니다.");
        console.error("CLI 인자로 설정하거나 .env 파일에 다음 변수를 설정해주세요:");
