...
// Register tools
server.tool("list_of_user_chats", "List of user chats in managed state", {
    state: z.enum(["opened", "closed"]),
}, async (input) => {
    return await listOfUserChatsHandler(input, ACCESS_KEY, SECRET_KEY);
});
server.tool("retrieve_list_of_messages_in_user_chats", "Retrieve list of messages in a user chats with parallel request", {
    user_chat_ids: z.array(z.string()),
}, async (input) => {
    return await retrieveListOfMessagesInUserChatsHandler(input, ACCESS_KEY, SECRET_KEY);
});
async function main() {
